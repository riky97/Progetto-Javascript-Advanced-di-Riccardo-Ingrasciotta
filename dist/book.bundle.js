...
async function get_description_book(ele) {
  let book = await axios__WEBPACK_IMPORTED_MODULE_0___default().get(`https://openlibrary.org${ele}.json`).then(response => {
  ...
  if (book.covers !== undefined) {
    ...
    for (let i = 0; i < JSON.parse(sessionStorage.getItem("collectionBook")).length; i++) {
  ...
  let img = await axios__WEBPACK_IMPORTED_MODULE_0___default().get(`https://covers.openlibrary.org/b/id/${id_img}-L.jpg`, {
